...
void draw_loop(int height, int width, int symbol) {
  ...
  char direction = -1;
  for (int x = 0.5 * width - 1 + WIDTH_IS_ODD; // middle
       (x < width) && (x >= 0);                // out of bounds check
       x--) {
    for (int y = (direction == 1)
                     ? 0
                     : height - 1; // whether to start from top or bottom
      ...
      // there was a check not to draw the second point if width is odd and x ==
      // 0, but mvaddch macro already does that for me
